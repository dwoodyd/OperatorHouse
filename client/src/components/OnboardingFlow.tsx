/* =============================================================================
   ...
   3 fullscreen cards, swipe/arrow navigation, skip + "Enter the House" CTA.
        ...
        return (
          <g key={stage}>
            ...
            {Array.from({ length: count }).map((_, j) => (
              <g key={j}>
                <rect
                  x={x + 2} y={46 + j * 38} width={52} height={30} rx="3"
...
export default function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  const goTo = useCallback((next: number, dir: "left" | "right") => {
    setAnimDir(dir === "right" ? "out-left" : "out-right");
    setTimeout(() => {
      setCard(next);
      setAnimDir("in");
    }, 280);
  ...
  const slideStyle: React.CSSProperties = {
    ...
    transform: animDir === "in" ? "translateX(0)" : animDir === "out-left" ? "translateX(-32px)" : "translateX(32px)",
  ...
  return (
    <div
      style={{
        ...
        transition: exiting ? "opacity 500ms ease" : "opacity 400ms ease",
    ...
    >
      ...
      {/* Card content */}
      <div style={{ ...slideStyle, width: "100%", maxWidth: 480, padding: "0 32px", textAlign: "center" }}>
        {/* Visual */}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 40 }}>
        ...
        {card < CARDS.length - 1 ? (
          ...
          <button
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 36px rgba(212,168,83,0.55)"; }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 24px rgba(212,168,83,0.35)"; }}
          >
